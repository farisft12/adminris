    ...
    <div class="relative flex min-h-screen items-center justify-center px-4 py-12">
        <div class="w-full max-w-md">
            ...
            <div class="animate-scale-in rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl opacity-0-init" style="animation-fill-mode: forwards;">
                <div class="mb-8">
                    <h2 class="text-2xl font-bold tracking-tight text-white">Buat akun baru</h2>
                    <p class="mt-1 text-slate-400">Isi data berikut untuk mendaftar.</p>
                </div>
                <form method="POST" action="{{ route('register') }}" class="space-y-5">
                    @csrf
                    <div class="animate-fade-in-up opacity-0-init" style="animation-delay: 0.05s; animation-fill-mode: forwards;">
                        <label for="name" class="mb-1.5 block text-sm font-medium text-slate-300">Nama</label>
                        <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus
                            class="input-guest w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-slate-500 shadow-inner transition-all duration-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/30 @error('name') border-red-400/50 focus:border-red-400 focus:ring-red-400/30 @enderror"
                    ...
                    <div class="animate-fade-in-up opacity-0-init" style="animation-delay: 0.2s; animation-fill-mode: forwards;">
                        ...
                        <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password"
                            class="input-guest w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-slate-500 shadow-inner transition-all duration-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/30"
                            placeholder="••••••••" />
                    </div>
                    <div class="animate-fade-in-up opacity-0-init" style="animation-delay: 0.25s; animation-fill-mode: forwards;">
                        <label for="role" class="mb-1.5 block text-sm font-medium text-slate-300">Role</label>
                        <select id="role" name="role" class="input-guest w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white shadow-inner transition-all duration-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/30">
                            <option value="staff" {{ old('role', 'staff') === 'staff' ? 'selected' : '' }}>Staff</option>
                            <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Admin</option>
                        </select>
                    </div>
                    <div class="animate-fade-in-up opacity-0-init pt-1" style="animation-delay: 0.3s; animation-fill-mode: forwards;">
                ...
                <p class="mt-6 animate-fade-in-up text-center text-sm text-slate-400 opacity-0-init" style="animation-delay: 0.35s; animation-fill-mode: forwards;">
                    Sudah punya akun?
                    <a href="{{ route('login') }}" class="font-medium text-indigo-400 transition-colors hover:text-indigo-300">Masuk</a>
                </p>
            </div>
        </div>
    </div>
