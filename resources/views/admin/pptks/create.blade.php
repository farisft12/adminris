...
<div class="mx-auto max-w-lg px-4 py-6 sm:px-6 lg:px-8">
    ...
    <h1 class="mt-2 text-2xl font-bold text-slate-900">Tambah PPTK</h1>
    <form method="POST" action="{{ route('admin.pptks.store') }}" class="mt-6 rounded-xl border border-slate-200 bg-white p-6">
        @csrf
        <div class="space-y-4">
            <div>
                <label for="nama_pptk" class="mb-1 block text-sm font-medium text-slate-700">Nama PPTK</label>
                <input id="nama_pptk" type="text" name="nama_pptk" value="{{ old('nama_pptk') }}" required class="block w-full rounded-lg border border-slate-300 px-3 py-2" />
            ...
            <div>
                ...
                <input id="nip" type="text" name="nip" value="{{ old('nip') }}" class="block w-full rounded-lg border border-slate-300 px-3 py-2" />
            </div>
        </div>
