    ...
    <div class="mx-auto max-w-xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="mb-10 animate-fade-up">
            ...
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Tahun <span class="text-slate-400">{{ $year->tahun }}</span></h1>
            </div>
        </div>

        <form method="POST" action="{{ route('admin.years.update', $year) }}" class="animate-fade-up border border-slate-200 bg-white rounded-2xl shadow-sm p-8" style="animation-delay: 100ms">
            @csrf @method('PUT')
            <div class="space-y-6">
                <div>
                    <label for="tahun" class="block text-[11px] font-black uppercase tracking-widest text-slate-500 mb-2 ml-1">Tahun Anggaran</label>
                    <input id="tahun" type="number" name="tahun" min="2000" max="2100" value="{{ old('tahun', $year->tahun) }}" required
